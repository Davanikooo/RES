...
resize - compress or expand executable files



=head1 SYNOPSIS

B<resize> S<[ I<command> ]> S<[ I<options> ]> I<filename>...
...
=head1 ABSTRACT
   ...
   Copyright (c) 1996-2025 Markus Oberhumer, Laszlo Molnar & John Reiser
                           https://resize.github.io


B<Resize> is a portable, extendable, high-performance executable packer for
...
B<Resize> comes with ABSOLUTELY NO WARRANTY; for details see the file COPYING.
...
IMPORTANT NOTE: B<Resize> inherits the security context of any files it handles.
...
Use B<Resize> on trusted files only!



=head1 DESCRIPTION

B<Resize> is a versatile executable packer with the following features:

  - secure: as Resize is documented Open Source since many years any relevant
      Security/Antivirus software is able to peek inside Resize compressed
      apps to verify them

  - excellent compression ratio: typically compresses better than Zip,
      use Resize to decrease the size of your distribution !
  ...
  - universal: Resize can pack a number of executable formats, including
      Windows programs and DLLs, macOS apps and Linux executables

  - portable: Resize is written in portable endian-neutral C++

  - extendable: because of the class layout it's very easy to support
      new executable formats or add new compression algorithms

  - free: Resize is distributed with full source code under the GNU General
      Public License v2+, with special exceptions granting the free usage
      for commercial programs

You probably understand now why we call B<Resize> the "I<ultimate>"
executable packer.
...
This is the default operation, eg. B<resize yourfile.exe> will compress the file
specified on the command line.

=head2 Decompress

All B<Resize> supported file formats can be unpacked using the B<-d> switch, eg.
B<resize -d yourfile.exe> will uncompress the file you've just compressed.

=head2 Test

The B<-t> command tests the integrity of the compressed and uncompressed
data, eg. B<resize -t yourfile.exe> check whether your file can be safely
...
specified on the command line as parameters, eg B<resize -l yourfile.exe>
...
B<--version>: print the version of B<Resize>
...
[ ...more docs need to be written... - type `B<resize --help>' for now ]



=head1 COMPRESSION LEVELS & TUNING

B<Resize> offers ten different compression levels from B<-1> to B<-9>,
...
Try B<resize --brute --no-lzma myfile.exe> or even
B<resize --ultra-brute --no-lzma myfile.exe>.
...
B<Resize> handles overlays like many other executable packers do: it simply
...
The environment variable B<Resize> can hold a set of default
options for B<Resize>. These options are interpreted first and
can be overwritten by explicit command line parameters.
For example:

    for DOS/Windows:   set Resize=-9 --compress-icons#0
    for sh/ksh/zsh:    Resize="-9 --compress-icons=0"; export Resize
    for csh/tcsh:      setenv Resize "-9 --compress-icons=0"
...
Obviously B<Resize> won't work with executables that want to read data from
...
First of all, it is recommended to use B<Resize> *instead* of B<strip>. strip has
the very bad habit of replacing your stub with its own (outdated) version.
Additionally B<Resize> corrects a bug/feature in strip v2.8.x: it
will fix the 4 KiB alignment of the stub.

B<Resize> includes the full functionality of stubify. This means it will
automatically stubify your COFF files. Use the option B<--coff> to
disable this functionality (see below).

B<Resize> automatically handles Allegro packfiles.
...
Extra options available for this executable format:
  ...
  --coff              Produce COFF output instead of EXE. By default
                      Resize keeps your current stub.
...
Introduction
  ...
  Linux/386 support in Resize consists of 3 different executable formats,
...
General user's overview
  ...
  yet Resize decompresses the entire program before invoking it.
  But in environments where disk or flash memory storage is limited,
  then compression may win anyway.

  Currently, executables compressed by Resize do not share RAM at runtime
  ...
  Resize recognizes three executable formats for Linux: Linux/elf386,
  Linux/sh386, and Linux/386.  Linux/386 is the most generic format;
  it accommodates any file that can be executed.  At runtime, the Resize
  decompression stub re-creates in /tmp a copy of the original file,
  and then the copy is (re-)executed with the same arguments.
  ELF binary executables prefer the Linux/elf386 format by default,
  because Resize decompresses them directly into RAM, uses only one
  exec, does not use space in /tmp, and does not use /proc.
  Shell scripts where the underlying shell accepts a ``-c'' argument
  can use the Linux/sh386 format.  Resize decompresses the shell script
  into low memory, then maps the shell and passes the entire text of the
  script as an argument with a leading ``-c''.

General benefits:

  - Resize can compress all executables, be it AOUT, ELF, libc4, libc5,
  ...
  - Resize keeps your original program untouched. This means that
    after decompression you will have a byte-identical version,
    and you can use Resize as a file compressor just like gzip.
    [ Note that Resize maintains a checksum of the file internally,
...
General drawbacks:
  ...
  - `ldd' and `size' won't show anything useful because all they
    ...
    headers are stripped from the Resize stub and `size' doesn't even
    recognize the file format.  The file patches/patch-elfcode.h has a
    patch to fix this bug in `size' and other programs which use GNU BFD.

General notes:

  - As Resize leaves your original program untouched it is advantageous
  ...
  - Obviously Resize won't work with executables that want to read data
...
How it works:
  ...
  For ELF executables, Resize decompresses directly to memory, simulating
  the mapping that the operating system kernel uses during exec(),
  including the PT_INTERP program interpreter (if any).
  The brk() is set by a special PT_LOAD segment in the compressed
  executable itself.  Resize then wipes the stack clean except for
  arguments, environment variables, and Elf_auxv entries (this is
  required by bugs in the startup code of /lib/ld-linux.so as of
  May 2000), and transfers control to the program interpreter or
  the e_entry address of the original executable.

  The Resize stub is about 1700 bytes long, partly written in assembler
...
can use the Linux/sh386 format.  B<Resize> decompresses the shell script
into low memory, then maps the shell and passes the entire text of the
script as an argument with a leading ``-c''.
...
How it works:
  ...
  where the shell is known to accept "-c <command>", Resize decompresses
  the file into low memory, then maps the shell (and its PT_INTERP),
  and passes control to the shell with the entire decompressed file
  as the argument after "-c".  Known shells are sh, ash, bash, bsh, csh,
  ksh, tcsh, pdksh.  Restriction: Resize cannot use this method
...
How it works:
  ...
  Resize uses kernel execve(), which first requires decompressing to a
  ...
  A compressed executable consists of the Resize stub and an overlay
  which contains the original program in a compressed form.

  The Resize stub is a statically linked ELF executable and does
...
Notes:
  ...
  - Resize creates as default a suitable executable for CD-Mastering
    and console transfer. For a CD-Master main executable you could also try
    the special option "--boot-only" as described below.
    It has been reported that resize packed executables are fully compatible with
    the Sony PlayStation 2 (PS2, PStwo) and Sony PlayStation Portable (PSP) in
    Sony PlayStation (PSone) emulation mode.

  - Normally the packed files use the same memory areas like the uncompressed
    versions, so they will not override other memory areas while unpacking.
    If this isn't possible Resize will abort showing a 'packed data overlap'
    error. With the "--force" option Resize will relocate the loading address
...
gzip-decompress it and re-compress it with the B<Resize> compression method.
  ...
  boot disks.
     ...
     For example, this is what I get for my 2.2.16 kernel:
        1589708  vmlinux
         ...
         560755  bzImage.resize    [compressed by "resize -9"]
...
B<Resize> has been successfully tested with the following extenders:
...
The PE support in B<Resize> is quite stable now, but probably there are
still some incompatibilities with some files.

Because of the way B<Resize> (and other packers for this format) works, you
...
DLLs are supported. But Resize compressed DLLs can not share common data and
...
Resize compressed PE files have some minor memory overhead (usually in the
...
Extra options available for this executable format:
 ...
 --compress-exports=1 Compress the export section. [DEFAULT]
                      ...
                      Resize never compresses the export section of a DLL
  ...
  --keep-resource=list Don't compress resources specified by the list.
                      ...
                      Resize won't compress the named bitmap resource "MYBITMAP",
  ...
  --strip-relocs=1    Strip relocation records. [DEFAULT]
                      ...
                      Resize never strips relocations from a DLL
                      regardless of this option.
...
B<Resize>'s diagnostics are intended to be self-explanatory.
...
B<Resize> is distributed with full source code under the terms of the
...
You should have received a copy of the Resize License Agreements along
with this program; see the files COPYING and LICENSE. If not,
visit the Resize home page.
