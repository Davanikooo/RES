...
macro(resize_cmake_include_hook section)
...
macro(resize_disallow_in_source_build)
...
macro(resize_ternary result_var_name condition true_value false_value)
    if(${condition})
...
macro(resize_set_default_build_type type)
    set(resize_global_default_build_type "${type}")
    get_property(resize_global_is_multi_config GLOBAL PROPERTY GENERATOR_IS_MULTI_CONFIG)
    if(NOT resize_global_is_multi_config AND NOT CMAKE_BUILD_TYPE)
        set(CMAKE_BUILD_TYPE "${resize_global_default_build_type}" CACHE STRING "Choose the type of build." FORCE)
    endif()
...
macro(resize_apply_build_type)
    if(resize_global_is_multi_config)
        set(c "${CMAKE_CONFIGURATION_TYPES}")
        list(INSERT c 0 "${resize_global_default_build_type}")
        if(CMAKE_BUILD_TYPE)
    ...
    if(NOT CMAKE_TRY_COMPILE_CONFIGURATION)
        if(CMAKE_BUILD_TYPE)
            ...
            set(CMAKE_TRY_COMPILE_CONFIGURATION "${resize_global_default_build_type}")
        endif()
    endif()
endmacro()

# set MINGW, MSVC_FRONTEND, GNU_FRONTEND and MSVC_SIMULATE
macro(resize_set_global_vars)
...
macro(resize_check_working_build_rpath var_name)
    if(WIN32 OR MINGW OR CYGWIN)
        # always works; DLLs reside next to the executables
        set(${var_name} ON)
        resize_cache_bool_vars(ON ${var_name})
    elseif(CMAKE_BUILD_WITH_INSTALL_RPATH OR CMAKE_SKIP_RPATH OR CMAKE_SKIP_BUILD_RPATH)
        ...
        set(${var_name} OFF)
        resize_cache_bool_vars(OFF ${var_name})
    else()
...
function(resize_make_bool_var result_var_name var_name default_value)
...
function(resize_maybe_unused_var) # ARGV
...
function(resize_print_var) # ARGV
    foreach(var_name ${ARGV})
        if(DEFINED ${var_name})
            if(NOT ",${${var_name}}," STREQUAL ",,")
                if(${var_name})
...
function(resize_print_env_var) # ARGV
    foreach(var_name ${ARGV})
...
function(resize_print_have_symbol) # ARGV; needs include(CheckSymbolExists)
...
function(resize_print_common_symbols)
    resize_print_have_symbol(__FAST_MATH__)
    resize_print_have_symbol(__PIC__ __pic__ __PIE__ __pie__)
endfunction()

# examine MinGW/Cygwin compiler configuration
function(resize_print_mingw_symbols)
    if(WIN32 OR MINGW OR CYGWIN)
        if(CMAKE_C_COMPILER_ID MATCHES "(Clang|GNU)")
            # runtime library: msvcrt vs ucrt vs cygwin
            resize_print_have_symbol(__CRTDLL__ __CYGWIN__ __CYGWIN32__ __CYGWIN64__ __MINGW32__ __MINGW64__ __MINGW64_VERSION_MAJOR __MSVCRT__ __MSVCRT_VERSION__ _UCRT _WIN32 _WIN32_WINNT _WIN64)
            # exception handing: SJLJ (setjmp/longjmp) vs DWARF vs SEH
            resize_print_have_symbol(__GCC_HAVE_DWARF2_CFI_ASM __SEH__ __USING_SJLJ_EXCEPTIONS__)
            # threads: win32 vs posix/pthread/winpthreads vs mcfgthread
            resize_print_have_symbol(_REENTRANT __USING_MCFGTHREAD__)
        endif()
    endif()
endfunction()

# add wildcard expansions to a variable
function(resize_add_glob_files) # ARGV
    set(var_name ${ARGV0})
    ...
    #message(STATUS "resize_add_glob_files: ${var_name} = ${result}")
    set(${var_name} "${result}" PARENT_SCOPE) # return value
endfunction()

# remove wildcard expansions from a variable
function(resize_remove_glob_files) # ARGV
    set(var_name ${ARGV0})
    list(REMOVE_AT ARGV 0)
    file(GLOB files ${ARGV})
    set(result "")
    ...
    #message(STATUS "resize_remove_glob_files: ${var_name} = ${result}")
    set(${var_name} "${result}" PARENT_SCOPE) # return value
endfunction()

# useful for CI jobs: allow settings via environment and cache result
function(resize_cache_bool_vars) # ARGV
    set(default_value "${ARGV0}")
    list(REMOVE_AT ARGV 0)
    foreach(var_name ${ARGV})
        set(value ${default_value})
        if(DEFINED RESIZE_CACHE_VALUE_${var_name})     # check cache
            set(value "${RESIZE_CACHE_VALUE_${var_name}}")
        ...
        elseif(DEFINED ENV{${var_name}})            # check environment
            if("$ENV{${var_name}}" MATCHES "^(0|1|OFF|ON|FALSE|TRUE|off|on|false|true)$")
                ...
                set(RESIZE_CACHE_ORIGIN_FROM_ENV_${var_name} TRUE CACHE INTERNAL "" FORCE) # for status message below
            endif()
        endif()
        # convert to bool
        if(value)
        ...
        # store result
        if(RESIZE_CACHE_ORIGIN_FROM_ENV_${var_name})
            message(STATUS "setting from environment: ${var_name} = ${value}")
        endif()
        set(${var_name} "${value}" PARENT_SCOPE) # store result
        set(RESIZE_CACHE_VALUE_${var_name} "${value}" CACHE INTERNAL "" FORCE) # and store in cache
    endforeach()
endfunction()

function(resize_platform_check_mismatch var_name_1 var_name_2)
    if(DEFINED ${var_name_1} OR DEFINED ${var_name_2})
        if(NOT ",${${var_name_1}}," STREQUAL ",${${var_name_2}},")
            if(RESIZE_CONFIG_DISABLE_WERROR)
...
function(resize_platform_check_c_cxx_mismatch)
    resize_platform_check_mismatch(CMAKE_C_PLATFORM_ID CMAKE_CXX_PLATFORM_ID)
    resize_platform_check_mismatch(CMAKE_C_SIMULATE_ID CMAKE_CXX_SIMULATE_ID)
    resize_platform_check_mismatch(CMAKE_C_COMPILER_ABI CMAKE_CXX_COMPILER_ABI)
    resize_platform_check_mismatch(CMAKE_C_COMPILER_FRONTEND_VARIANT CMAKE_CXX_COMPILER_FRONTEND_VARIANT)
endfunction()
...
function(resize_internal_add_definitions_with_prefix) # ARGV; needs include(CheckCCompilerFlag)
...
function(resize_add_definitions) # ARGV; needs include(CheckCCompilerFlag)
    set(failed_flags "")
    if(MSVC_FRONTEND AND CMAKE_C_COMPILER_ID MATCHES "Clang")
        # for clang-cl try "-clang:" flag prefix first
        resize_internal_add_definitions_with_prefix("-clang:" ${ARGV})
        resize_internal_add_definitions_with_prefix("empty" ${failed_flags})
    else()
        resize_internal_add_definitions_with_prefix("empty" ${ARGV})
    endif()
endfunction()

# useful for CI jobs: allow target extra compile options
function(resize_add_target_extra_compile_options) # ARGV
    set(t "${ARGV0}")
    list(REMOVE_AT ARGV 0)
    foreach(var_name ${ARGV})
...
function(resize_compile_target_debug_with_O2) # ARGV
    foreach(t ${ARGV})
        if(MSVC_FRONTEND)
...
function(resize_compile_source_debug_with_O2) # ARGV
    set(flags "$<$<CONFIG:Debug>:-O2>")
    if(${CMAKE_VERSION} VERSION_LESS "3.8")
        # 3.8: The COMPILE_FLAGS source file property learned to support generator expressions
        if(resize_global_is_multi_config OR NOT CMAKE_BUILD_TYPE MATCHES "^Debug$")
            return()
        endif()
        set(flags "-O2")
    endif()
...
function(resize_sanitize_target) # ARGV
    if(NOT DEFINED RESIZE_CONFIG_SANITIZE_FLAGS_DEBUG)
        # default sanitizer for Debug builds
        set(RESIZE_CONFIG_SANITIZE_FLAGS_DEBUG -fsanitize=undefined -fsanitize-undefined-trap-on-error -fstack-protector-all)
    endif()
    if(NOT DEFINED RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE)
        # default sanitizer for Release builds
        set(RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE -fstack-protector)
    endif()
    foreach(t ${ARGV})
        if(RESIZE_CONFIG_DISABLE_SANITIZE)
            # no-op
        ...
        elseif(CMAKE_C_COMPILER_ID MATCHES "^GNU" AND CMAKE_C_COMPILER_VERSION VERSION_LESS "8.0")
            ...
            target_compile_options(${t} PRIVATE $<$<CONFIG:Debug>:${RESIZE_CONFIG_SANITIZE_FLAGS_DEBUG}>)
            target_compile_options(${t} PRIVATE $<$<CONFIG:MinSizeRel>:${RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE}>)
            target_compile_options(${t} PRIVATE $<$<CONFIG:Release>:${RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE}>)
            target_compile_options(${t} PRIVATE $<$<CONFIG:RelWithDebInfo>:${RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE}>)
            get_target_property(target_type ${t} TYPE)
            if(NOT ",${target_type}," STREQUAL ",STATIC_LIBRARY,")
            if(${CMAKE_VERSION} VERSION_GREATER "3.12.99")
            target_link_options(${t} PRIVATE $<$<CONFIG:Debug>:${RESIZE_CONFIG_SANITIZE_FLAGS_DEBUG}>)
            target_link_options(${t} PRIVATE $<$<CONFIG:MinSizeRel>:${RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE}>)
            target_link_options(${t} PRIVATE $<$<CONFIG:Release>:${RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE}>)
            target_link_options(${t} PRIVATE $<$<CONFIG:RelWithDebInfo>:${RESIZE_CONFIG_SANITIZE_FLAGS_RELEASE}>)
            endif() # CMAKE_VERSION
            endif() # target_type
        endif()
    endforeach()
...
function(resize_test_depends) # ARGV
    set(name "${ARGV0}")
    list(REMOVE_AT ARGV 0)
