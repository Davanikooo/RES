...
#undef CHECK_TYPE_PAIR
    ...
    // check RES_VERSION_xxx
    {
        using upx::compile_time::mem_eq;
        using upx::compile_time::string_len;
        static_assert(string_len(RES_VERSION_STRING4) == 4);
        static_assert(string_len(RES_VERSION_YEAR) == 4);
        static_assert(string_len(RES_VERSION_DATE_ISO) == 10);
        static_assert(string_len(RES_VERSION_DATE) == 12 || string_len(RES_VERSION_DATE) == 13);
        static_assert(mem_eq(RES_VERSION_STRING4, RES_VERSION_STRING, 3));
        static_assert(mem_eq(RES_VERSION_YEAR, RES_VERSION_DATE_ISO, 4));
        static_assert(mem_eq(RES_VERSION_YEAR, &RES_VERSION_DATE[sizeof(RES_VERSION_DATE) - 5], 4));
        char buf[16];
        constexpr long long v = RES_VERSION_HEX;
        upx_safe_snprintf(buf, sizeof(buf), "%lld.%lld.%lld", (v >> 16), (v >> 8) & 255, v & 255);
        assert_noexcept(strcmp(buf, RES_VERSION_STRING) == 0);
        upx_safe_snprintf(buf, sizeof(buf), "%lld.%lld%lld", (v >> 16), (v >> 8) & 255, v & 255);
        assert_noexcept(strcmp(buf, RES_VERSION_STRING4) == 0);
    }
